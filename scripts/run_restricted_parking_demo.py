...
def main():
        ...
        for track in tracks:
            ...
            for zone in config.zones:
                ...
                if evidence is not None:
                    contact_debug[(track.track_id,zone.zone_id)]={"frame_index":frame_index,"track_id":track.track_id,
                        "vehicle_class":tracker.display_class(track.track_id),"zone_id":zone.zone_id,
                        "inside_policy":evidence.restricted_overlap>=config.minimum_restricted_overlap,
                        "motion_score":engine.motion_score(track.track_id,zone.zone_id),
                        "stationary_duration_s":engine.stationary_seconds(track.track_id,zone.zone_id,source_time),
                        "restricted_overlap":evidence.restricted_overlap,"surface_class":evidence.surface_class,
                        "surface_confidence":evidence.surface_confidence,"state":engine.state(track.track_id,zone.zone_id).value,
                        "states_seen":sorted(state.value for state,identifiers in state_tracks.items() if track.track_id in identifiers),
                        "ground_contact_bbox":ground_contact_region(b,width,height,config.ground_contact_fraction)}
                    break
    ...
    print("state_track_counts:",json.dumps({state.value:len(identifiers) for state,identifiers in state_tracks.items()}))
