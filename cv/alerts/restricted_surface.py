...
class RestrictedSurfaceResolver:
    ...
    def evaluate(self,bbox,restricted,semantic,*,width,height,ground_contact_fraction,minimum_overlap):
        ...
        raw=None if self.mode is RestrictedSurfaceMode.POLICY_DEFINED else "sidewalk" if occupied and "SIDEWALK" in self.restricted_classes else None
        surface_class="POLICY_DEFINED" if self.mode is RestrictedSurfaceMode.POLICY_DEFINED else "SIDEWALK" if raw else "OTHER"
        return RestrictedSurfaceEvidence(occupied,low_surface or near,overlap,surface_confidence,surface_class,raw,tuple(reasons))
