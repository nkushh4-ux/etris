...
class HuggingFaceSegFormerProvider:
    """Offline SegFormer provider with label-name based ETRIS surface mapping."""

    def __init__(self,weights:str|Path,*,device="cuda",input_size=(512,512),
        road_labels=("road",),sidewalk_labels=("sidewalk",),_model=None,_processor=None):
        import torch
        ...
        by_name={name.strip().casefold():index for index,name in id2label.items()}
        requested_road={name.strip().casefold() for name in road_labels}
        requested_sidewalk={name.strip().casefold() for name in sidewalk_labels}
        if not requested_road or not requested_sidewalk:
            raise ValueError("road_labels and sidewalk_labels must not be empty")
        missing=sorted((requested_road|requested_sidewalk)-by_name.keys())
        if missing:
            raise ValueError(f"SegFormer checkpoint is missing configured labels: {', '.join(missing)}")
        self.road_class_ids=frozenset(by_name[name] for name in requested_road)
        self.sidewalk_class_ids=frozenset(by_name[name] for name in requested_sidewalk)
...
class HuggingFaceSegFormerCityscapesProvider(HuggingFaceSegFormerProvider):
    """Existing Cityscapes mapping retained for backward compatibility."""


IDD_ROAD_LABELS=("road","parking","drivable fallback")
IDD_SIDEWALK_LABELS=("sidewalk","non-drivable fallback")


class HuggingFaceSegFormerIDDProvider(HuggingFaceSegFormerProvider):
    """IDD mapping based on checkpoint label names, never numeric class indexes."""

    def __init__(self,weights:str|Path,*,device="cuda",input_size=(512,512),
        road_labels=IDD_ROAD_LABELS,sidewalk_labels=IDD_SIDEWALK_LABELS,
        _model=None,_processor=None):
        super().__init__(weights,device=device,input_size=input_size,road_labels=road_labels,
            sidewalk_labels=sidewalk_labels,_model=_model,_processor=_processor)
